...
static int ptPlayDoom(struct pt *pt)
{
  // Start a protothread
  static unsigned long lastTimePlayed = 0;
  static int currentDoomNote = 0;

  PT_BEGIN(pt);

  if (currentDoomNote < (notes * 2)) 
  {
    // stop the waveform generation before the next note.
    noTone(buzzer);
    
    // calculates the duration of each note
    divider = pgm_read_word_near(melody + currentDoomNote + 1);
    if (divider > 0) 
    {
      // regular note, just proceed
      noteDuration = (wholenote) / divider;
    ...
    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(buzzer, pgm_read_word_near(melody + currentDoomNote), noteDuration * 0.9);

    lastTimePlayed = millis();

    // Wait for the specief duration before playing the next note.
    PT_WAIT_UNTIL(pt, millis() - lastTimePlayed > noteDuration * 1.06);
    
    currentDoomNote = currentDoomNote + 2;
  }
